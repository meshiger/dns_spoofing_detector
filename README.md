...
3. **TTL Inspection**: Detects abnormally high TTL values often used by attackers to "poison" the local DNS cache for extended periods.
4. **Duplicate Detection**: Identifies if multiple responses are received for a single query ID—a signature of an attacker attempting to "outrun" the legitimate DNS server.
